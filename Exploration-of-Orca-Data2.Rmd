...
data_Gen <- function(n){
  ...
  for(i in 1:n){ #generate pod sizes
    ...
    pod_data<- pod() %>% #add lactating females
      mutate(number_repro_females = round(pod_size * 0.22,0))
    Lactating_females <- c()
    for(i in 1:nrow(pod_data)){
      Lactating_females[i] <- rbinom(1,pod_data$number_repro_females[i], prob = 0.2)
...
```{r}
beta <- 0.05 #significant to 0.05
power_poisson <- function(beta){
  sims <- 100 #simulation number
  pVal <- c(); SalmonEffect <- c()
  for(i in 1:sims){ #loop for the number of simulations
    data <- data_Gen(31) #generate the orca data for 31 years (1980-2010)
    #make a glm
    model <- glmer(lactating_females~total_salmon+
                   (1|group_number)+(1|year), 
                 family = poisson, data)
    #save the effect salmon has
    SalmonEffect[i] <- summary(model)$coefficient[2,1]
    pVal[i] <- summary(model)$coefficient[2,4] #save model significance
  }
  powEst <- length(which(pVal<beta))/length(pVal)
  return(list(SalmonEffect, powEst))
}

results <- power_poisson(beta)
print(paste("Power Estimate: ", results[2]))
minEffect <- min(as.data.frame(results[1]))
maxEffect <- max(as.data.frame(results[1]))
print(paste("Range of Salmon Effects: ", minEffect, "-", maxEffect))
```
