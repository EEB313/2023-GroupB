...
power_poisson <- function(beta){
  ...
  for(i in 1:sims){ #loop for the number of simulations
    ...
    predictions[[i]] <- cbind(lact_fem_predict=
                                predict(models[[i]], 
                                        newdata = data.frame(
                                          total_salmon=gen_Data$total_salmon,
                                          pod_size = gen_Data$pod_size,
                                          year=gen_Data$year,
                                          group_number=gen_Data$group_number)), 
                              total_salmon = gen_Data$total_salmon,
                              year = gen_Data$year,
                              pod_size = gen_Data$pod_size,
                              group_number = gen_Data$group_number,
...
print(paste("Salmon Power Estimate: ", results[4]))
minSaEffect <- min(as.data.frame(results[1]))
maxSaEffect <- max(as.data.frame(results[1]))
print(paste("Range of Salmon Effects: ", minSaEffect, "-", maxSaEffect))
print(paste("Pod Power Estimate: ", results[5]))
minPodEffect <- min(as.data.frame(results[2]))
maxPodEffect <- max(as.data.frame(results[2]))
...
seq(min(as.data.frame(results[[6]])$pod_size),
    max(as.data.frame(results[[6]])$pod_size), length = 1000) -> pods
...
for (i in 1:length(results[[3]])){
  lines1 <- as.data.frame(cbind(salmons = salmons)) %>%
    mutate(lact_prediction = results[[3]][i]+
             results[[1]][i]*salmons)
